...
function generateFluentSchema (schema) {
  for (const key of SCHEMAS_SOURCE) {
    if (schema[key] && (schema[key].isFluentSchema || schema[key].isFluentJSONSchema || schema[key][kFluentSchema])) {
  ...
  if (schema.response) {
    ...
    for (const code of httpCodes) {
      if (schema.response[code].isFluentSchema || schema.response[code].isFluentJSONSchema || schema.response[code][kFluentSchema]) {
